...
class TalentMap extends Component
{
    ...
    public function getSkillStats()
    {
        try {
            ...
            $totalMembers = IgrejaMembro::where('status', 'ativo')->count();
            $membersWithSkills = DB::table('habilidades_membros')->distinct('membro_id')->count('membro_id');
            $totalSkills = DB::table('habilidades_membros')->count();
